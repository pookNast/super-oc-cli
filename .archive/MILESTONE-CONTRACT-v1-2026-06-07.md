# Milestone Contract — OpenCode Feature Extraction into oc-start
**Generated:** 2026-06-07  
**Owner:** pookNast + Claude Code  
**Source repo:** github.com/anomalyco/opencode (TypeScript monorepo)  
**Target:** `~/bin/oc-start` + `~/openclaude-workspace/`  
...
OpenCode is structured as `packages/core` (session, agents, tools, permissions), `packages/llm` (provider routes, model catalog), and `packages/tui` (Solid.js terminal UI). We extract patterns, not wholesale packages.
...
| # | Deliverable | Acceptance Criteria |
|---|-------------|-------------------|
| 3.1 | Agent profile schema: name, mode (primary/subagent), model, tools, system prompt | JSON/YAML config per agent in `~/.openclaude/agents/` |
...
| 8.2 | Per-session token accumulator | Running total displayed on `/cost` command |
